...
tree=DT.DecisionNode('d1', dict_decisions=dictDecisions, dict_chances=dictChances, dict_terminals=dictTerminals)
...
non_surg_survive=DT.ChanceNode('No_Surg_Survive', cum_prob=(D.Pr_Access*(1-D.Pr_Surg)*(1-D.Pr_NoSurg_Die)),
                               dict_chances=dictChances, dict_terminals=dictTerminals)
non_surg_maj_comp=DT.ChanceNode('No_S_Major_Comp',
                                cum_prob=(D.Pr_Access*(1-D.Pr_Surg)*(1-D.Pr_NoSurg_Die)*D.Pr_NoS_Maj_Comp),
                                dict_chances=dictChances, dict_terminals=dictTerminals)
...
# we will probably need to modify this but this should work for now
PST = (1 - D.Pr_Die)
pComp = D.Pr_Maj_Comp
YLD_comp = D.YLD_comp
RD_postTX = D.Pr_Die_Comp
YLL = D.YLL
YLD_disease = D.YLD_disease
DALY = (PST*(pComp*YLD_comp)) + ((1-PST)*((RD_postTX*(YLL+YLD_disease))+((1-RD_postTX)*YLD_disease)))
